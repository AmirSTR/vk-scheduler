...
# Conversation states
(WAIT_MESSAGE, WAIT_PEER_ID, WAIT_DATETIME, WAIT_REPEAT) = range(4)
...
def schedule_job(task: dict):
    ...
    async def job_func():
        ...
        # Если не повторяется — удаляем после выполнения
    ...
    if task['repeat_type'] == 'once':
        ...
        # Повтор каждые N часов
        hours = task['repeat_value']
        scheduler.add_job(
            job_func, IntervalTrigger(hours=hours),
            id=job_id, replace_existing=True,
            next_run_time=datetime.fromisoformat(task['next_run'])
        )

    elif task['repeat_type'] == 'daily':
        # Каждый день в определённое время HH:MM
        ...
        # Каждую неделю: "1:14:30" = понедельник в 14:30
...
async def add_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "Для беседы это число из ссылки + 2000000000\n"
        "Например: беседа /convo/4 → peer_id = 2000000004\n\n"
        "Введи peer_id:"
    )
    return WAIT_PEER_ID
...
async def add_peer_id(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "🕐 Введи дату и время первой отправки в формате:\n"
        "ДД.ММ.ГГГГ ЧЧ:ММ\n\n"
        "Например: 25.04.2025 14:30\n"
        "(Время московское)"
    )
...
async def add_datetime(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        await update.message.reply_text("❌ Неверный формат. Используй ДД.ММ.ГГГГ ЧЧ:ММ\nНапример: 25.04.2025 14:30")
        return WAIT_DATETIME

    keyboard = [
        [InlineKeyboardButton("🔂 Раз (без повтора)", callback_data="repeat:once:")],
        [InlineKeyboardButton("🔁 Каждые N часов", callback_data="repeat:interval:")],
...
async def add_repeat(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    _, repeat_type, repeat_value = query.data.split(':', 2)
    context.user_data['repeat_type'] = repeat_type

    if repeat_type == 'once':
        await _save_task(query, context, repeat_type, '')
        return ConversationHandler.END

    elif repeat_type == 'interval':
        context.user_data['pending_repeat'] = repeat_type
        await query.edit_message_text("Каждые сколько часов отправлять? Введи число (например: 24):")
        return WAIT_REPEAT

    elif repeat_type == 'daily':
        # Берём время из next_run
        dt = datetime.fromisoformat(context.user_data['next_run'])
        repeat_value = f"{dt.hour:02d}:{dt.minute:02d}"
        await _save_task(query, context, repeat_type, repeat_value)
        return ConversationHandler.END
...
async def add_repeat_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Обработка текстового ввода на этапе повтора (для interval)"""
    pending = context.user_data.get('pending_repeat')
    ...
    if pending == 'interval':
        try:
            ...
            if hours < 1:
                raise ValueError
            await _save_task_msg(update, context, 'interval', str(hours))
            return ConversationHandler.END
        except ValueError:
            await update.message.reply_text("❌ Введи целое число часов (например: 24):")
            return WAIT_REPEAT
...
async def add_weekday(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await _save_task(query, context, 'weekly', repeat_value)
    return ConversationHandler.END


async def _save_task(query, context, repeat_type, repeat_value):
    ...
    repeat_str = _repeat_label(repeat_type, repeat_value)
...
async def _save_task_msg(update, context, repeat_type, repeat_value):
    data = context.user_data
    task_id = db.add_task(
        message=data['message'],
        peer_id=data['peer_id'],
...
def _repeat_label(repeat_type, repeat_value):
    if repeat_type == 'once':
        return 'Без повтора'
    elif repeat_type == 'interval':
        return f'Каждые {repeat_value} ч.'
    elif repeat_type == 'daily':
...
async def list_tasks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for t in tasks:
        ...
        text += (
            ...
            f"   🔄 {_repeat_label(t['repeat_type'], t['repeat_value'])}\n\n"
        )
...
def main():
    ...
    add_conv = ConversationHandler(
        ...
        states={
            ...
            WAIT_REPEAT: [
                CallbackQueryHandler(add_repeat, pattern='^repeat:'),
                CallbackQueryHandler(add_weekday, pattern='^day:'),
                MessageHandler(filters.TEXT & ~filters.COMMAND, add_repeat_text),
            ],
        },
